...
local function totemName(spellName, rank)
	...
	return spellName .. suffix
end

local totemIcons = {}

for _, spellId in ipairs(TOTEM_SPELL_IDS) do
	local spellName, rank, texture = GetSpellInfo(spellId)
	if spellName then
		totemIcons[totemName(spellName, rank)] = texture
	end
end
...
if GetLocale() == "ruRU" then
	local RU_NAMES = {
		...
		[58739] = "Тотем сопротивления огню",
		[58749] = "Тотем сопротивления силам природы",
	}
